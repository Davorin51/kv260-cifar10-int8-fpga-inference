...
typedef struct {
    ...
    XCifar10_cnn_uio_map maps[ MAX_UIO_MAPS ];
} XCifar10_cnn_uio_info;

/***************** Variable Definitions **************************************/
static XCifar10_cnn_uio_info uio_info;
...
static int uio_info_read_name(XCifar10_cnn_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCifar10_cnn_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCifar10_cnn_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCifar10_cnn_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCifar10_cnn_Initialize(XCifar10_cnn *InstancePtr, const char* InstanceName) {
	XCifar10_cnn_uio_info *InfoPtr = &uio_info;
